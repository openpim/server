...
export default {
    ...
    Mutation: {
        ...
        executeAction: async (parent: any, { itemId, actionIdentifier, data, serverUuid }: any, context: Context) => {
            context.checkAuth()

            if (serverUuid) {
                const currentServerUuid = ModelsManager.getInstance().getServerUuid()
                if (serverUuid === currentServerUuid) {
                    logger.debug(`Received action request for itself, skip it. ${serverUuid}`)
                    return {}
                }
                context.setRemoteActionExecution()
            }
