    ...
    for (const prop in values) {
        ...
        if (attr) {
            ...
            if (values[prop] && Array.isArray(values[prop])) {
                ...
                const tst = parseInt(values[prop])
                if (isNaN(tst)) throw new Error(`Wrong value for item: ${item.identifier} for relation attribute: ${prop} => ${JSON.stringify(values[prop])}`)
                relatedItemsIds.push(tst)
            }
        }
    }
...
export async function processItemActions(context: Context, event: EventType, item: Item, newParent: string, newName: string, newValues: any, newChannels: any, isImport: boolean, isFileUpload: boolean, fromRelationAttribute = false, transaction: Transaction | null = null, data: any = null) {
    ...
    return await processActions(mng, actions, {
        ...
        system: { fs, exec, awaitExec, fetch, URLSearchParams, mailer, nodemailerNTLMAuth, http, https, http2, moment, XLSX, archiver, stream, pipe, FS, KafkaJS, extractzip, HtmlValidate, uuid },
        isImport: isImport,
...
export async function testAction(context: Context, action: Action, item: Item) {
    ...
    const ret = await processActionsWithLog(mng, [action], {
        ...
        roles: context.getUser()?.getRoles(),
        utils: new ActionUtils(context),
        system: { fs, exec, awaitExec, fetch, URLSearchParams, mailer, nodemailerNTLMAuth, http, https, http2, XLSX, FS, pipe, stream, archiver, extractzip, HtmlValidate, uuid },
            ...
            if (script instanceof VMScript || script === undefined) {
                if (script === undefined) {
                    ...
                    try {
                        ...
                        logger.error(`Failed to compile script for action ${action.identifier}, error: "${err.message}"`);
